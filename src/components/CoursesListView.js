  ...
  return (
    <>
      ...
      <ul style={{ padding: '0 5%', margin: '0' }}>
        {
          courses.map(course => <li key={course.id} className="row"><a href={course.certi} rel="noopener noreferrer" target="_blank" className="white-text link-unstyled course"><h5 className="title" style={{ fontSize: '1.20rem' }}>{course.title}</h5><span className="offeredBy left">{offeredBy}</span><span className="plateform right">{plateform}</span></a></li>)
        }
